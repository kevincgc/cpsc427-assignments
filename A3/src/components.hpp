...
struct Motion {
	...
	vec2 facing = { 0, 0 }; // unit vector in the direction the entity is facing
	int is_swimming = 0;
};

struct Physics {
	float mass = 0;
	float coefficient_of_resititution = 0.6;
	float radius = 0;
};

struct FeelsGravity {
	bool is_free_fall = true;
};
...
struct Debug {
	...
	bool is_advance_ai = 1;
	bool is_advance_physics = 0;
};
...
// DONE A1: Add a timer that will light the salmon up upon eating a fish
struct LightUp
{
